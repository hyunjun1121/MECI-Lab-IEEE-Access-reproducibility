...
The analysis was run from the immutable aggregate-analysis manifest and the verified local TRIBE release. The submission audit records the source hashes, the 0.05 alignment threshold, four-second model input clip, one-second persisted interval, three/five/ten-second window widths, analysis cutoff, timezone, seed, bootstrap count, null permutations, feature sets, and fold rules. The public GitHub repository and versioned Zenodo archive listed in the Data Availability statement provide the code, aggregate tables, figure files, configuration records, and checksums used to reproduce the reported analyses, following the findable, accessible, interoperable, and reusable data-stewardship principles for the aggregate artifacts \cite{wilkinson2016fair}. Raw media, model weights, runtime caches, raw Analytics responses, and private owner records are excluded.
...
Code, sanitized public metadata, derived aggregate analysis tables, figure files, configuration records, and checksums are available in the public GitHub repository and its versioned Zenodo archive: \url{https://github.com/hyunjun1121/MECI-Lab-IEEE-Access-reproducibility} and \url{https://doi.org/10.5281/zenodo.22890876}. Raw video and audio, raw YouTube Analytics responses, model weights, runtime caches, and other private owner-authorized records are not redistributed.
...
Generative artificial intelligence assistance from OpenAI Codex \cite{openAICodex} was used for language drafting and revision in the Abstract, Introduction, Related Work, Methods, Results, Discussion, Conclusion, and Appendix, and for code and document inspection and LaTeX/figure preparation. It was not used to create or alter source data, owner-authorized Analytics responses, model outputs, or statistical results. The authors independently verified all numerical results, citations, ethical and data-access statements, figures, and final wording and accept responsibility for the content.
